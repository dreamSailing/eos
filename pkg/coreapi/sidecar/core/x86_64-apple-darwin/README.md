# `x86_64-apple-darwin` vendored sidecar（macOS Intel）

此目录存放 macOS Intel（x86_64-apple-darwin）的 eos-core sidecar 二进制 + manifest.json。

## 填充方式

由 GitHub Actions workflow `sync-vendored-sidecar.yml` 在 **macos-13 原生 runner**
上编译 + Ed25519 签名 + 校验 sha256 后自动 vendored 进此目录。
