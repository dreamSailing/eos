...
func TestRuntimeEventFromEnvelopeNormalizesRustTurnEvent(t *testing.T) {
	event := runtimeEventFromEnvelope(protocol.Envelope{
		EventType: protocol.EventTypeTurnTextDelta,
		SessionID: "session-1",
		TurnID:    "turn-1",
		AgentID:   "agent-1",
	...
	if got, _ := event.Data["original_event_type"].(string); got != string(protocol.EventTypeTurnTextDelta) {
		t.Fatalf("original_event_type=%q, want %q", got, protocol.EventTypeTurnTextDelta)
	}
}
